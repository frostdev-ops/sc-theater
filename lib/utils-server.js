...
class LogManager {
    ...
    log(level, message, category = null, data = null) {
        ...
        if (level === 'error') {
            console.error(`[${timestamp.toISOString()}] [ERROR] ${category ? `[${category.toUpperCase()}] ` : ''}${message}`, data || '');
        }
    ...
    startSummaryTimer() {
        ...
        console.log(`[${new Date().toISOString()}] [INFO] Log summary interval started (${this.summaryInterval / 60000} minutes).`);
    }
