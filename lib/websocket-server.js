...
class WebSocketServer {
    ...
    handleConnection(ws, req) {
        ...
        // Properly handle X-Forwarded-For header which can be a comma-separated list
        // The client's real IP is typically the first one in the list
        let clientIp = req.socket.remoteAddress;
        
        if (req.headers['x-forwarded-for']) {
            // Extract the first IP from the X-Forwarded-For header (client's original IP)
            const forwardedIps = req.headers['x-forwarded-for'].split(',');
            clientIp = forwardedIps[0].trim();
        }
        
        this.logger.info(`Client ${clientId} connecting from ${clientIp}...`, 'connection');

        // Temporary info until authenticated
        const tempClientInfo = { ip: clientIp, ws: ws, id: clientId }; 

        const authTimeout = setTimeout(() => {
            if (!this.stateManager.getClient(clientId)) { // Check StateManager if client exists
        ...
        ws.on('close', (code, reason) => {
            ...
            this.handleDisconnect(clientId, code, reason ? reason.toString() : 'Unknown Reason');
        });
    ...
    async handleMessage(ws, message, tempClientInfo, authTimeout) {
        ...
        const clientId = tempClientInfo.id;
        try {
            parsedMessage = JSON.parse(message);
            if (!parsedMessage.type || typeof parsedMessage.type !== 'string') {
                throw new Error('Invalid message format: Missing or invalid "type"');
            }
            this.logger.debug(`Received msg type '${parsedMessage.type}' from ${clientIp} (${clientId})`, 'websocket');
        } catch (e) {
        ...
        if (parsedMessage.type === 'auth') {
             if (this.stateManager.getClient(clientId)) {
            ...
            this.stateManager.addClient(clientId, clientInfo);

            // Send auth success message, including the new client ID
            this.send(ws, { type: 'auth_success', clientId: clientId, ...authResult });

            // Send initial state immediately
            const currentClientInfo = this.stateManager.getClient(clientId);
        ...
        const clientInfo = this.stateManager.getClientById(ws);
        if (!clientInfo) {
    ...
    handleDisconnect(clientId, code, reason) {
        const clientInfo = this.stateManager.getClient(clientId);
        if (clientInfo) {
            ...
            this.broadcastViewerListToAdmins(ws); // Notify admins (excluding the disconnected ws)
             ...
             this.logger.info(`Unauthenticated client with ID ${clientId} disconnected. Code: ${code}, Reason: ${reason}`, 'connection');
        }
    }
    ...
    send(ws, data) {
        if (ws.readyState === WebSocket.OPEN) {
            try {
                ...
                this.handleDisconnect(clientId, 1011, 'Send Error'); // Manually handle disconnect
                return false;
            }
        }
        return false;
            ...
            if (ws !== excludeWs && ws.readyState === WebSocket.OPEN) {
                ...
                try {
                    ...
                    this.logger.error(`Error broadcasting message to client:`, error, 'websocket');
                     ...
                     const clientInfo = this.stateManager.getClientById(ws);
                     const clientId = clientInfo ? clientInfo.id : 'unknown';
                     this.handleDisconnect(clientId, 1011, 'Broadcast Send Error');
                }
            }
        });
    }
    ...
    broadcastState(excludeWs = null) {
        ...
        this.wss.clients.forEach((clientWs) => {
             if (clientWs !== excludeWs && clientWs.readyState === WebSocket.OPEN) {
                 const clientInfo = this.stateManager.getClientById(clientWs);
    ...
    scheduleNextSync(ws, clientInfo) {
        ...
        if (this.stateManager.masterState.isPlaying) {
             clientInfo.syncTimerId = setTimeout(() => {
                 ...
                 const currentClientInfo = this.stateManager.getClient(clientInfo.id);
    ...
    sendViewerList(adminWs) {
        const adminInfo = this.stateManager.getClientById(adminWs);
    ...
    broadcastViewerListToAdmins(excludeWs = null) {
        ...
        this.stateManager.connectedClients.forEach((info, clientId) => {
             if (info.role === 'admin' && info.ws !== excludeWs && info.ws.readyState === WebSocket.OPEN) {
                 this.send(info.ws, message);
             }
        });
    }
    ...
    handleClientTimeUpdate(ws, clientInfo, message) {
        ...
        this.stateManager.updateClientInfo(clientInfo.id, updates);
        
        // Adjust this client's sync interval based on the new drift
        const intervalChanged = this.stateManager.adjustClientSyncInterval(clientInfo.id, this.stateManager.getClient(clientInfo.id)); 
        if (intervalChanged) {
            this.scheduleNextSync(ws, this.stateManager.getClient(clientInfo.id)); // Reschedule immediately if interval changed
        }
    ...
    checkHeartbeats() {
        ...
        this.stateManager.connectedClients.forEach((clientInfo, clientId) => {
            const ws = clientInfo.ws;
            if (ws.readyState === WebSocket.OPEN) {
