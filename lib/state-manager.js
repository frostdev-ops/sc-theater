...
class StateManager {
    constructor(logger, options = {}) {
        ...
        // Map<WebSocket, { role, ip, name, token, lastDrift, lastReportedTime, isPlaying, playbackRate, currentSyncInterval, syncTimerId, missedHeartbeats }>
    ...
    addClient(ws, clientInfo) {
        ...
        this.connectedClients.set(ws, { ...defaultClientState, ...clientInfo });
    ...
    getClient(ws) {
        return this.connectedClients.get(ws);
    }

    updateClientInfo(ws, updates) {
        if (this.connectedClients.has(ws)) {
            const currentInfo = this.connectedClients.get(ws);
            this.connectedClients.set(ws, { ...currentInfo, ...updates });
            return true;
        }
        return false;
    }
    ...
    getAllClientsInfo() {
        ...
        this.connectedClients.forEach((info, ws) => {
            if (ws.readyState === WebSocket.OPEN) { // Ensure socket is open
                 clientsArray.push({
                     role: info.role,
                     name: info.name,
                     ip: info.ip,
    ...
    stopRateAdjustment() {
     ...
     adjustClientSyncInterval(ws, clientInfo) {
        ...
        if (intervalAdjusted && newInterval !== clientInfo.currentSyncInterval) {
            ...
            this.updateClientInfo(ws, { currentSyncInterval: newInterval });
